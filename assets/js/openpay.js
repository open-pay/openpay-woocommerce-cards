...
jQuery(document).ready(function () {
  ...
  jQuery("body").append(
    '<div id="card-points-dialog" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; z-index:999999; align-items:center; justify-content:center;">' +
      /* Fondo oscurecido */
      '<div style="position:absolute; width:100%; height:100%; background:rgba(0,0,0,0.6);"></div>' +
      /* Caja del Modal */
      '<div style="position:relative; background:#fff; padding:30px; border-radius:10px; max-width:400px; width:90%; text-align:center; box-shadow:0 5px 15px rgba(0,0,0,0.3); font-family:sans-serif;">' +
      '<h4 style="margin:0 0 15px; font-size:20px; color:#333;">Pagar con Puntos</h4>' +
      '<div style="margin-bottom:25px; font-size:15px; color:#666; line-height:1.5;">' +
      "<p>¿Desea usar los puntos de su tarjeta para realizar este pago?</p>" +
      "</div>" +
      '<div style="display:flex; gap:12px; justify-content:center;">' +
      '<button type="button" id="points-yes-button" style="background:#5cb85c; color:#fff; border:none; padding:12px 25px; border-radius:5px; cursor:pointer; font-weight:bold; font-size:14px;">Si</button>' +
      '<button type="button" id="points-no-button" style="background:#eeeeee; color:#333; border:none; padding:12px 25px; border-radius:5px; cursor:pointer; font-weight:bold; font-size:14px;">No</button>' +
      "</div>" +
      "</div>" +
      "</div>"
  );
  ...
  jQuery(document).on("change", "#openpay_month_interest_free", function () {
    ...
    if (months > 1) {
      jQuery("#total-monthly-payment").show();
    } else {
  ...
  jQuery(document).on("change", "#openpay_selected_card", function () {
    ...
    if (jQuery("#openpay_selected_card").val() !== "new") {
      ...
      if (country === "PE") {
        openpay_params.installments.paymentPlan
  ...
  jQuery("body").on("click", "form.checkout button:submit", function () {
    ...
    if (
      jQuery("input[name=payment_method]:checked").val() == "wc_openpay_gateway"
    ) {
  ...
  function CardsErrorHandler(save_cc_option) {
    ...
    if (
      jQuery("#openpay_selected_card").val() == "new" &&
      (jQuery("#openpay-holder-name").val().length < 1 ||
        !pattern.test(jQuery("#openpay-holder-name").val()))
    ) {
    ...
    if (
      jQuery("#openpay_selected_card").val() !== "new" &&
      jQuery("#openpay-card-cvc").val().length < 3 &&
      save_cc_option === "1"
    ) {
  ...
  jQuery("form#order_review").submit(function () {
    ...
    if (jQuery("#openpay_selected_card").val() !== "new") {
      $form.append(
        '<input type="hidden" name="openpay_token" value="' +
          jQuery("#openpay_selected_card").val() +
          '" />'
      );
      return true;
    }
  ...
  function success_callback(response) {
    ...
    $form.append(
      '<input type="hidden" name="openpay_token" value="' + token + '" />'
    );
    $form.append(
      '<input type="hidden" name="openpay_tokenized_card" value="' +
        card_number +
        '" />'
    );
    console.log("card points: " + response.data.card.points_card);
    if (
      openpay_params.country == "MX" &&
      response.data.card.points_card &&
      openpay_params.use_card_points
    ) {
      // Si la tarjeta permite usar puntos, mostrar el cuadro de diálogo
      jQuery("#card-points-dialog").css("display", "flex").hide().fadeIn();
    } else {
  ...
  jQuery("#points-yes-button").on("click", function (e) {
    e.preventDefault();
    e.stopPropagation();
    console.log("Puntos: SI");
    jQuery("#openpay_card_points_confirm").val("true");
    jQuery("#card-points-dialog").fadeOut();
    // Enviamos el formulario manualmente
    jQuery("form.checkout, form#order_review").submit();
  });

  jQuery("#points-no-button").on("click", function (e) {
    e.preventDefault();
    e.stopPropagation();
    console.log("Puntos: NO");
  ...
  function error_callback(response) {
    ...
    switch (response.data.error_code) {
      ...
      case 2006:
        msg =
          "El código de seguridad de la tarjeta (CVV2) no fue proporcionado o es incorrecto";
        break;

      case 1:
        msg =
          "El nombre del titular de la tarjeta no fue proporcionado o tiene un formato inválido.";
        break;
    ...
    jQuery("#openpay_selected_card")
      .closest("div")
      .before(
        '<ul style="background-color: #e2401c; color: #fff;" class="woocommerce_error woocommerce-error"><li> ERROR ' +
  ...
  jQuery("body").on("keyup", "#openpay-card-number", function () {
    ...
    if (card_without_space.length == 8) {
      if (
        (country == "MX" && !openpay_params.installments.payments) ||
        (country == "PE" && !openpay_params.installments.paymentPlan)
      ) {
  ...
  function getTypeCard(cardBin, country) {
    jQuery.ajax({
      ...
      success: function (response) {
        ...
        if (response.status == "success") {
          if (response.card_type === "CREDIT") {
            if (country == "MX")
              jQuery("#openpay_month_interest_free").closest("p, div").show();
            else jQuery("#openpay_installments").closest("p, div").show();
          } else if (
            ...
            jQuery("#openpay_installments_pe").append(
              jQuery("<option>", {
                value: 1,
                text: "Solo una cuota",
              })
            ...
            jQuery("#openpay_installments_pe").closest("p, div").show();
            ...
            if (country == "MX") {
              jQuery("#openpay_month_interest_free").closest("p, div").hide();
              jQuery('#openpay_month_interest_free option[value="1"]').attr(
                "selected",
                true
              );
              jQuery("#total-monthly-payment").hide();
            } else {
              jQuery("#openpay_installments").closest("p, div").hide();
              jQuery('#openpay_installments option[value="1"]').attr(
                "selected",
                true
              );
              jQuery("#openpay_installments_pe").closest("p, div").hide();
              jQuery('#openpay_installments_pe option[value="1"]').attr(
                "selected",
                true
              );
            }
      ...
      complete: function () {
        jQuery("#openpay_cards").css("opacity", "1");
