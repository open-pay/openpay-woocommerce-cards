jQuery(document).ready(function () {
  ...
  function showOrHideElements(country) {
    var chargeMX = jQuery(
      "#woocommerce_wc_openpay_gateway_charge_type",
    ).closest("tr");
    var chargeCOPE = jQuery(
      "#woocommerce_wc_openpay_gateway_charge_type_co_pe",
    ).closest("tr");
    ...
    if (country == "PE" || country == "CO") {
      ...
      jQuery("#woocommerce_wc_openpay_gateway_minimum_amount_interest_free")
        .closest("tr")
        .hide();
      ...
      if (country == "PE") {
        jQuery(
          "#woocommerce_wc_openpay_gateway_save_card_mode option[value='2']",
        ).show();
  ...
  if (jQuery("#woocommerce_wc_openpay_gateway_sandbox").length) {
    ...
    jQuery("#woocommerce_wc_openpay_gateway_sandbox").on(
      "change",
      function (e) {
        is_sandbox();
      },
